...
const spendPoints = (pointsToSpend, transactionHistory) => {
  ...
  for (const transaction of sortedHistory) {
    ...
    if (
      Object.values(negativeBalance).every((v) => v === 0) &&
      pointBalance === 0
    ) {
      let result = [];
    ...
    if (negativeBalance[payer] < 0) {
      pointBalance = handleNegativeBalance(
        transaction,
        pointBalance,
        negativeBalance,
        spentBalance
...
const recordNegativeBalance = (transactionHistory, negativeBalance) => {
  for (const transaction of transactionHistory) {
    if (transaction.points < 0) {
      negativeBalance[transaction.payer] =
        (negativeBalance[transaction.payer] || 0) + transaction.points;
    }
  }
};
